...
//Разрешена ли вообще на сайте регистрация
...
// if(!$result['MESSAGE'] && !$formData['UF_POLICY']){
// 	$result['MESSAGE'] = "Не введено значение обязательного поля. Политика конфиденциальности.";
// }
if(!$result['MESSAGE']) {
	...
	if(intval($userId) > 0) {
		...
		if($bConfirmEmail) {
            ...
            CEvent::Send("API_AUTH_NEW_USER", SITE_ID, $userData);
		}

	...
	<div id="<?=$formId?>_wrap" class="api-auth-register" data-css="<?=$dataCss?>" data-js="<?=$dataJs?>">
		<form id="<?=$formId?>"
				...
				<div class="api_row api-row-privacy api-row-accept">
					<div class="api_controls">
						<div class="api-accept-label">
							<input type="checkbox"
							       name="PRIVACY_ACCEPTED" id="reg_policy"
							       value="Y">
							...
							<?/*<script>
								$(document).ready(function(){
									$("html").on('click', '.api-accept-text', function(){
										if($(".api-row-privacy .jcf-checkbox").hasClass('jcf-checked')){
											$(".api-row-privacy .jcf-checkbox").addClass('jcf-unchecked');
											$(".api-row-privacy .jcf-checkbox").removeClass('jcf-checked');
											$("#reg_policy").prop('checked', false);
										}else{
											$(".api-row-privacy .jcf-checkbox").addClass('jcf-checked');
											$(".api-row-privacy .jcf-checkbox").removeClass('jcf-unchecked');
											$("#reg_policy").prop('checked', true);
										}
									});
								});
							</script>
			...
			<? if($arResult['DISPLAY_USER_CONSENT']): ?>
						...
						<? foreach($arResult['DISPLAY_USER_CONSENT'] as $agreementId => $arAgreement): ?>
							<div class="api_control">
								<div class="api-accept-label" data-id="<?=$agreementId?>">
									<input type="checkbox"
									       name="USER_CONSENT_ID[]" id="reg_policy"
									       value="<?=$agreementId?>">
				...
				<script>
					$(document).ready(function(){
						$("html").on('click', '.api-row-user-consent .api-accept-text', function(){
							if($(".api-accept-label .jcf-checkbox").hasClass('jcf-checked')){
								$(".api-accept-label .jcf-checkbox").addClass('jcf-unchecked');
								$(".api-accept-label .jcf-checkbox").removeClass('jcf-checked');
								$("#reg_policy").prop('checked', false);
							}else{
								$(".api-accept-label .jcf-checkbox").addClass('jcf-checked');
								$(".api-accept-label .jcf-checkbox").removeClass('jcf-unchecked');
								$("#reg_policy").prop('checked', true);
							}
						});
					});
			...
			<div class="api_row api_grid api_grid_width_1_2">
				<div>
					<a class="api_link api_auth_register_url auth_fancybox_link"
						data-fancybox
					   	href="#modal-entrance"
					   	data-header="<?=CUtil::JSEscape($arParams['~LOGIN_MESS_HEADER'])?>"><?=Loc::getMessage('API_AUTH_REGISTER_LOGIN_URL')?></a>
				</div>
				<div class="api_text_right">
					<a class="api_link api_auth_restore_url auth_fancybox_link"
						data-fancybox
					   	href="#modal-restore"

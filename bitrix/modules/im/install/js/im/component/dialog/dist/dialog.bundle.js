(function (exports,main_polyfill_intersectionobserver,ui_vue,ui_vue_vuex,im_component_message,im_const,im_utils,im_tools_animation) {
	'use strict';
	...
	ui_vue.Vue.component('bx-messenger-dialog', {
	  ...
	  created: function created() {
	    ...
	    this.readMessageDelayed = im_utils.Utils.debounce(this.readMessage, 50, this);
	    this.requestHistoryBlockIntersect = false;
	    this.requestHistoryDelayed = im_utils.Utils.debounce(this.requestHistory, 50, this);
	    this.requestUnreadBlockIntersect = false;
	    this.requestUnreadDelayed = im_utils.Utils.debounce(this.requestUnread, 50, this);
	  ...
	  mounted: function mounted() {
	    ...
	    this.windowFocused = im_utils.Utils.platform.isBitrixMobile() ? true : document.hasFocus();
	  },
	    ...
	    statusReaded: function statusReaded() {
	      ...
	      if (this.dialog.type === im_const.DialogType.private) {
	        ...
	        if (record.messageId === this.lastMessageId && record.userId !== this.lastMessageAuthorId) {
	          var dateFormat = im_utils.Utils.date.getFormatType(BX.Messenger.Const.DateFormat.readedTitle, this.$root.$bitrixMessages);
	  ...
	  methods: {
	    ...
	    scrollTo: function scrollTo() {
	      ...
	      if (this.animateScrollId) {
	        im_tools_animation.Animation.cancel(this.animateScrollId);
	      ...
	      this.animateScrollId = im_tools_animation.Animation.start({
	        start: start,
	        end: end,
	        increment: increment,
	        duration: duration,
	    ...
	    _groupTitle: function _groupTitle(date) {
	      ...
	      var dateFormat = im_utils.Utils.date.getFormatType(BX.Messenger.Const.DateFormat.groupTitle, this.$root.$bitrixMessages);
	...
	var Utils = {
	  ...
	  scrollToPosition: function scrollToPosition(context, position) {
	    ...
	    if (context.animateScrollId) {
	      im_tools_animation.Animation.cancel(context.animateScrollId);
	      this.scrollAnimating = false;
...
}((this.window = this.window || {}),BX,BX,BX,window,BX.Messenger.Const,BX.Messenger,BX.Messenger));

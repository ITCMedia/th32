...
if ($arParams['SHOW_MAIN_PAGE'] == 'Y'){
	$arrTempResult = array();
	$numberTempSect = 0;

	foreach ($arResult['SECTIONS'] as $key => &$arSection)
	{
		if ($arSection['UF_SHOW_FOR_MAIN'] == '1') {
			$arrTempResult[$numberTempSect] = $arSection;
			$numberTempSect++;
		}
	}

	$arResult['SECTIONS'] = $arrTempResult;
	$arResult['SECTIONS_COUNT'] = $numberTempSect;
}
